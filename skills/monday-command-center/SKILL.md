...
Use the plugin-root `scripts/monday_system.py publish` command for a validated plan and `status` to verify freshness and native-app readback. A successful file write is `published`, not `displayed`. Represent the cockpit as current only when the app acknowledges the same `planID` and schema version.
